...
public class HUDMod extends Mod {
	...
	public void drawRoundedImage(File file, float addX, float addY, float width, float height, float radius, float alpha) {
		Glide instance = Glide.getInstance();
		if (instance == null || file == null) return;
	...
	public void drawBackground(float addX, float addY, float width, float height, float radius) {
		...
		if (nvg == null || colorManager == null) return;
		
		AccentColor currentColor = colorManager.getCurrentColor();
		if (currentColor == null) return;
		
		InternalSettingsMod internalSettings = InternalSettingsMod.getInstance();
		...
		if (isNormal || isVanilla || isShadow || isDark || isLight || isModern) {
			nvg.drawShadow(finalX, finalY, lastWidth, lastHeight, radius - 0.75F);
		} else if (isGlow || isVanillaGlow) {
			nvg.drawGradientShadow(finalX, finalY, lastWidth, lastHeight, radius, currentColor.getColor1(), currentColor.getColor2());
		} else if (isOutline || isOutlineGlow) {
			if (isOutline) {
				nvg.drawShadow(finalX - 2, finalY - 2, lastWidth + 4, lastHeight + 4, radius + 2);
			} else if (isOutlineGlow) {
				nvg.drawGradientShadow(finalX - 2, finalY - 2, lastWidth + 4, lastHeight + 4, radius + 2, currentColor.getColor1(), currentColor.getColor2());
			}
		}

		if (isOutline || isOutlineGlow) {
			nvg.drawGradientOutlineRoundedRect(finalX - 1, finalY - 1, lastWidth + 2, lastHeight + 2, radius + 1, 1.5F, currentColor.getColor1(), currentColor.getColor2());
		}

		if (isVanilla || isVanillaGlow || isOutline || isOutlineGlow) {
			nvg.drawRoundedRect(finalX, finalY, lastWidth, lastHeight, radius, new Color(0, 0, 0, 100));
		} else if (isNormal || isGlow) {
			nvg.drawGradientRoundedRect(finalX, finalY, lastWidth, lastHeight, radius, ColorUtils.applyAlpha(currentColor.getColor1(), 220), ColorUtils.applyAlpha(currentColor.getColor2(), 220));
		} else if (isLight) {
			nvg.drawRoundedRect(finalX, finalY, lastWidth, lastHeight, radius, new Color(240, 240, 240, 220));
		} else if (isDark) {
			nvg.drawRoundedRect(finalX, finalY, lastWidth, lastHeight, radius, new Color(20, 20, 20, 220));
		}
		
		if (isRect || isSimpGrad) {
			nvg.drawRect(finalX, finalY, lastWidth, lastHeight, new Color(20, 20, 20, 165));
		}
		
		if (isSimpGrad) {
			nvg.drawHorizontalGradientRect(finalX, finalY - (2 * scale), lastWidth, (2 * scale), ColorUtils.interpolateColors(8, 0, currentColor.getColor1(), currentColor.getColor2()), ColorUtils.interpolateColors(10, 20, currentColor.getColor1(), currentColor.getColor2()));
		}
		
		if (isModern) {
			nvg.drawRoundedRect(finalX, finalY, lastWidth, lastHeight, radius, new Color(0, 0, 0, 110));
			nvg.drawOutlineRoundedRect(finalX - 0.5F, finalY - 0.5F, lastWidth + 1, lastHeight + 1, radius + 0.5F, 0.7F, new Color(255, 255, 255, 110));
		}
	}
	...
	public Color getFontColor(int alpha) {
		...
		if (isDark || isLight) {
			Glide instance = Glide.getInstance();
			if (instance == null) return new Color(255, 255, 255, alpha);
