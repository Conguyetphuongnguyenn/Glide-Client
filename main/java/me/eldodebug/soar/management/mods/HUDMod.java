package me.eldodebug.soar.management.mods;

import java.awt.Color;
import java.io.File;

import eu.shoroa.contrib.render.ShBlur;
import me.eldodebug.soar.Glide;
import me.eldodebug.soar.gui.GuiEditHUD;
import me.eldodebug.soar.management.color.AccentColor;
import me.eldodebug.soar.management.color.ColorManager;
import me.eldodebug.soar.management.language.TranslateText;
import me.eldodebug.soar.management.mods.impl.InternalSettingsMod;
import me.eldodebug.soar.management.mods.settings.impl.ComboSetting;
import me.eldodebug.soar.management.mods.settings.impl.combo.Option;
import me.eldodebug.soar.management.nanovg.NanoVGManager;
import me.eldodebug.soar.management.nanovg.font.Font;
import me.eldodebug.soar.management.nanovg.font.Fonts;
import me.eldodebug.soar.utils.ColorUtils;
import net.minecraft.util.ResourceLocation;

public class HUDMod extends Mod {

	private static final float DEFAULT_SCALE = 1.0F;
	private static final Color COLOR_VANILLA_BG = new Color(0, 0, 0, 100);
	private static final Color COLOR_LIGHT_BG = new Color(240, 240, 240, 220);
	private static final Color COLOR_DARK_BG = new Color(20, 20, 20, 220);
	private static final Color COLOR_RECT_BG = new Color(20, 20, 20, 165);
	private static final Color COLOR_MODERN_BG = new Color(0, 0, 0, 110);
	private static final Color COLOR_MODERN_OUTLINE = new Color(255, 255, 255, 110);
	private static final Color COLOR_TEXT_SHADOW = new Color(0, 0, 0, 150);
	
	private static Glide glideInstance;
	private static NanoVGManager nvgManager;
	private static ColorManager colorManager;
	private static InternalSettingsMod internalSettings;
	
	static {
		ensureInitialized();
	}
	
	private static void ensureInitialized() {
		if(glideInstance == null) {
			glideInstance = Glide.getInstance();
		}
		if(glideInstance != null) {
			if(nvgManager == null) {
				nvgManager = glideInstance.getNanoVGManager();
			}
			if(colorManager == null) {
				colorManager = glideInstance.getColorManager();
			}
			if(internalSettings == null) {
				internalSettings = InternalSettingsMod.getInstance();
			}
		}
	}
	
	private int x, y, draggingX, draggingY, width, height;
	private float scale;
	private boolean dragging, draggable;

	public HUDMod(TranslateText nameTranslate, TranslateText descriptionText) {
		super(nameTranslate, descriptionText, ModCategory.HUD);
		this.x = 100;
		this.y = 100;
		this.width = 100;
		this.height = 100;
		this.scale = DEFAULT_SCALE;
		this.draggable = true;
	}

	public HUDMod(TranslateText nameTranslate, TranslateText descriptionText, String alias) {
		super(nameTranslate, descriptionText, ModCategory.HUD, alias);
		this.x = 100;
		this.y = 100;
		this.width = 100;
		this.height = 100;
		this.scale = DEFAULT_SCALE;
		this.draggable = true;
	}

	public HUDMod(TranslateText nameTranslate, TranslateText descriptionText, String alias, boolean restricted) {
		super(nameTranslate, descriptionText, ModCategory.HUD, alias, restricted);
		this.x = 100;
		this.y = 100;
		this.width = 100;
		this.height = 100;
		this.scale = DEFAULT_SCALE;
		this.draggable = true;
	}

	public void save() {
		if(nvgManager == null) {
			ensureInitialized();
			if(nvgManager == null) return;
		}
		nvgManager.save();
	}

	public void restore() {
		if(nvgManager == null) {
			ensureInitialized();
			if(nvgManager == null) return;
		}
		nvgManager.restore();
	}

	public void scissor(float addX, float addY, float width, float height) {
		if(nvgManager == null) {
			ensureInitialized();
			if(nvgManager == null) return;
		}
		nvgManager.scissor(x + (addX * scale), y + (addY * scale), width * scale, height * scale);
	}

	public void drawPlayerHead(ResourceLocation location, float addX, float addY, float width, float height, float radius) {
		if(location == null || nvgManager == null) {
			ensureInitialized();
			if(nvgManager == null) return;
		}
		nvgManager.drawPlayerHead(location, x + (addX * scale), y + (addY * scale), width * scale, height * scale, radius * scale);
	}

	public void drawRoundedImage(int texture, float addX, float addY, float width, float height, float radius) {
		if(nvgManager == null) {
			ensureInitialized();
			if(nvgManager == null) return;
		}
		nvgManager.drawRoundedImage(texture, x + (addX * scale), y + (addY * scale), width * scale, height * scale, radius * scale);
	}

	public void drawRoundedImage(File file, float addX, float addY, float width, float height, float radius, float alpha) {
		if(file == null || nvgManager == null) {
			ensureInitialized();
			if(nvgManager == null) return;
		}
		nvgManager.drawRoundedImage(file, x + (addX * scale), y + (addY * scale), width * scale, height * scale, radius * scale, alpha);
	}

	public void drawRoundedImage(File file, float addX, float addY, float width, float height, float radius) {
		drawRoundedImage(file, addX, addY, width, height, radius, 1.0F);
	}

	public void drawRoundedImage(ResourceLocation location, float addX, float addY, float width, float height, float radius) {
		if(location == null || nvgManager == null) {
			ensureInitialized();
			if(nvgManager == null) return;
		}
		nvgManager.drawRoundedImage(location, x + (addX * scale), y + (addY * scale), width * scale, height * scale, radius * scale);
	}

	public void drawArc(float addX, float addY, float radius, float startAngle, float endAngle, float strokeWidth, Color color) {
		if(color == null || nvgManager == null) {
			ensureInitialized();
			if(nvgManager == null) return;
		}
		nvgManager.drawArc(x + (addX * scale), y + (addY * scale), radius * scale, startAngle, endAngle, strokeWidth * scale, color);
	}

	public void drawArc(float addX, float addY, float radius, float startAngle, float endAngle, float strokeWidth) {
		drawArc(addX, addY, radius, startAngle, endAngle, strokeWidth, this.getFontColor());
	}

	public void drawShadow(float addX, float addY, float width, float height, float radius) {
		if(nvgManager == null) {
			ensureInitialized();
			if(nvgManager == null) return;
		}
		nvgManager.drawShadow(x + (addX * scale), y + (addY * scale), width * scale, height * scale, radius * scale);
	}

	public void drawRect(float addX, float addY, float width, float height, Color color) {
		if(color == null || nvgManager == null) {
			ensureInitialized();
			if(nvgManager == null) return;
		}
		nvgManager.drawRect(x + (addX * scale), y + (addY * scale), width * scale, height * scale, color);
	}

	public void drawRect(float addX, float addY, float width, float height) {
		drawRect(addX, addY, width, height, this.getFontColor());
	}

	public void drawRoundedRect(float addX, float addY, float width, float height, float radius, Color color) {
		if(color == null || nvgManager == null) {
			ensureInitialized();
			if(nvgManager == null) return;
		}
		if(width < 0 || height < 0) return;
		nvgManager.drawRoundedRect(x + (addX * scale), y + (addY * scale), width * scale, height * scale, radius * scale, color);
	}

	public void drawRoundedRect(float addX, float addY, float width, float height, float radius) {
		drawRoundedRect(addX, addY, width, height, radius, this.getFontColor());
	}

	public void drawBackground(float addX, float addY, float width, float height, float radius) {
		if(nvgManager == null || colorManager == null) {
			ensureInitialized();
			if(nvgManager == null || colorManager == null) return;
		}
		
		AccentColor currentColor = colorManager.getCurrentColor();
		if(currentColor == null) return;
		
		if(internalSettings == null) {
			ensureInitialized();
			if(internalSettings == null) return;
		}
		
		ComboSetting setting = internalSettings.getModThemeSetting();
		if(setting == null) return;
		
		Option theme = setting.getOption();
		if(theme == null) return;

		boolean isNormal = theme.getTranslate().equals(TranslateText.NORMAL);
		boolean isVanilla = theme.getTranslate().equals(TranslateText.VANILLA);
		boolean isGlow = theme.getTranslate().equals(TranslateText.GLOW);
		boolean isVanillaGlow = theme.getTranslate().equals(TranslateText.VANILLA_GLOW);
		boolean isOutline = theme.getTranslate().equals(TranslateText.OUTLINE);
		boolean isOutlineGlow = theme.getTranslate().equals(TranslateText.OUTLINE_GLOW);
		boolean isShadow = theme.getTranslate().equals(TranslateText.SHADOW);
		boolean isDark = theme.getTranslate().equals(TranslateText.DARK);
		boolean isLight = theme.getTranslate().equals(TranslateText.LIGHT);
		boolean isRect = theme.getTranslate().equals(TranslateText.RECT);
		boolean isModern = theme.getTranslate().equals(TranslateText.MODERN);
		boolean isSimpGrad = theme.getTranslate().equals(TranslateText.GRADIENT_SIMPLE);
		boolean isBlur = internalSettings.getBlurSetting().isToggled();

		float lastWidth = width * scale;
		float lastHeight = height * scale;
		float finalX = this.x + (addX * scale);
		float finalY = this.y + (addY * scale);

		if(isBlur) {
			ShBlur.getInstance().drawBlur(finalX, finalY, lastWidth, lastHeight, radius);
		}

		if(isNormal || isVanilla || isShadow || isDark || isLight || isModern) {
			nvgManager.drawShadow(finalX, finalY, lastWidth, lastHeight, radius - 0.75F);
		} else if(isGlow || isVanillaGlow) {
			nvgManager.drawGradientShadow(finalX, finalY, lastWidth, lastHeight, radius, currentColor.getColor1(), currentColor.getColor2());
		} else if(isOutline || isOutlineGlow) {
			if(isOutline) {
				nvgManager.drawShadow(finalX - 2, finalY - 2, lastWidth + 4, lastHeight + 4, radius + 2);
			} else if(isOutlineGlow) {
				nvgManager.drawGradientShadow(finalX - 2, finalY - 2, lastWidth + 4, lastHeight + 4, radius + 2, currentColor.getColor1(), currentColor.getColor2());
			}
		}

		if(isOutline || isOutlineGlow) {
			nvgManager.drawGradientOutlineRoundedRect(finalX - 1, finalY - 1, lastWidth + 2, lastHeight + 2, radius + 1, 1.5F, currentColor.getColor1(), currentColor.getColor2());
		}

		if(isVanilla || isVanillaGlow || isOutline || isOutlineGlow) {
			nvgManager.drawRoundedRect(finalX, finalY, lastWidth, lastHeight, radius, COLOR_VANILLA_BG);
		} else if(isNormal || isGlow) {
			nvgManager.drawGradientRoundedRect(finalX, finalY, lastWidth, lastHeight, radius, ColorUtils.applyAlpha(currentColor.getColor1(), 220), ColorUtils.applyAlpha(currentColor.getColor2(), 220));
		} else if(isLight) {
			nvgManager.drawRoundedRect(finalX, finalY, lastWidth, lastHeight, radius, COLOR_LIGHT_BG);
		} else if(isDark) {
			nvgManager.drawRoundedRect(finalX, finalY, lastWidth, lastHeight, radius, COLOR_DARK_BG);
		}
		
		if(isRect || isSimpGrad) {
			nvgManager.drawRect(finalX, finalY, lastWidth, lastHeight, COLOR_RECT_BG);
		}
		
		if(isSimpGrad) {
			nvgManager.drawHorizontalGradientRect(finalX, finalY - (2 * scale), lastWidth, (2 * scale), ColorUtils.interpolateColors(8, 0, currentColor.getColor1(), currentColor.getColor2()), ColorUtils.interpolateColors(10, 20, currentColor.getColor1(), currentColor.getColor2()));
		}
		
		if(isModern) {
			nvgManager.drawRoundedRect(finalX, finalY, lastWidth, lastHeight, radius, COLOR_MODERN_BG);
			nvgManager.drawOutlineRoundedRect(finalX - 0.5F, finalY - 0.5F, lastWidth + 1, lastHeight + 1, radius + 0.5F, 0.7F, COLOR_MODERN_OUTLINE);
		}
	}

	public void drawBackground(float width, float height) {
		drawBackground(0, 0, width, height, 6 * scale);
	}

	public void drawBackground(float addX, float addY, float width, float height) {
		drawBackground(addX, addY, width, height, 6 * scale);
	}

	public void drawBackground(float width, float height, float radius) {
		drawBackground(0, 0, width, height, radius);
	}

	public void drawText(String text, float addX, float addY, float size, Font font, Color color) {
		if(text == null || font == null || color == null) return;
		
		if(nvgManager == null || internalSettings == null) {
			ensureInitialized();
			if(nvgManager == null) return;
		}

		if(font == Fonts.MOJANGLES) {
			addX = addX - 0.5F;
			addY = addY - 1.3F;
		}

		float lastSize = size * scale;
		
		if(internalSettings == null) return;
		
		ComboSetting setting = internalSettings.getModThemeSetting();
		if(setting == null) return;
		
		Option theme = setting.getOption();
		if(theme != null) {
			boolean isText = theme.getTranslate().equals(TranslateText.TEXT);
			if(isText) {
				nvgManager.save();
				nvgManager.fontBlur(20);
				nvgManager.drawText(text, x + (addX * scale), y + (addY * scale), COLOR_TEXT_SHADOW, lastSize, font);
				nvgManager.restore();
			}
		}

		nvgManager.drawText(text, x + (addX * scale), y + (addY * scale), new Color(color.getRed(), color.getGreen(), color.getBlue(), 180), lastSize, font);
	}

	public void scale(float addX, float addY, float width, float height, float nvgScale) {
		if(nvgManager == null) {
			ensureInitialized();
			if(nvgManager == null) return;
		}
		nvgManager.scale(x + (addX * scale), y + (addY * scale), width * scale, height * scale, nvgScale);
	}

	public void drawText(String text, float addX, float addY, float size, Font font) {
		drawText(text, addX, addY, size, font, getFontColor());
	}

	public void drawCenteredText(String text, float addX, float addY, float size, Font font, Color color) {
		if(text == null || font == null || color == null) return;
		
		if(nvgManager == null) {
			ensureInitialized();
			if(nvgManager == null) return;
		}

		float lastSize = size * scale;
		nvgManager.drawCenteredText(text, x + (addX * scale), y + (addY * scale), color, lastSize, font);
	}

	public void drawCenteredText(String text, float addX, float addY, float size, Font font) {
		drawCenteredText(text, addX, addY, size, font, getFontColor());
	}

	public float getTextWidth(String text, float size, Font font) {
		if(text == null || font == null) return 0.0F;
		
		if(nvgManager == null) {
			ensureInitialized();
			if(nvgManager == null) return 0.0F;
		}

		return nvgManager.getTextWidth(text, size, font);
	}

	public Color getFontColor(int alpha) {
		if(internalSettings == null) {
			ensureInitialized();
			if(internalSettings == null) return new Color(255, 255, 255, alpha);
		}
		
		ComboSetting setting = internalSettings.getModThemeSetting();
		if(setting == null) return new Color(255, 255, 255, alpha);
		
		Option theme = setting.getOption();
		if(theme == null) return new Color(255, 255, 255, alpha);

		boolean isDark = theme.getTranslate().equals(TranslateText.DARK);
		boolean isLight = theme.getTranslate().equals(TranslateText.LIGHT);

		if(isDark || isLight) {
			if(colorManager == null) {
				ensureInitialized();
				if(colorManager == null) return new Color(255, 255, 255, alpha);
			}
			
			AccentColor currentColor = colorManager.getCurrentColor();
			if(currentColor == null) return new Color(255, 255, 255, alpha);
			
			return currentColor.getInterpolateColor(alpha);
		}

		return new Color(255, 255, 255, alpha);
	}

	public Color getFontColor() {
		return getFontColor(255);
	}

	public boolean isEditing() {
		return mc.currentScreen instanceof GuiEditHUD;
	}

	public int getX() { return x; }
	public void setX(int x) { this.x = x; }
	public int getY() { return y; }
	public void setY(int y) { this.y = y; }
	public int getDraggingX() { return draggingX; }
	public void setDraggingX(int draggingX) { this.draggingX = draggingX; }
	public int getDraggingY() { return draggingY; }
	public void setDraggingY(int draggingY) { this.draggingY = draggingY; }
	public int getWidth() { return (int) (width * scale); }
	public void setWidth(int width) { this.width = width; }
	public int getHeight() { return (int) (height * scale); }
	public void setHeight(int height) { this.height = height; }
	public boolean isDragging() { return dragging; }
	public void setDragging(boolean dragging) { this.dragging = dragging; }

	public float getScale() { return scale; }
	
	public void setScale(float scale) {
		if(scale > 5.0) {
			this.scale = 5.0F;
			return;
		}
		if(scale < 0.2) {
			this.scale = 0.2F;
			return;
		}
		this.scale = scale;
	}

	public boolean isDraggable() { return draggable; }
	public void setDraggable(boolean draggable) { this.draggable = draggable; }

	public Font getHudFont(int in) {
		if(internalSettings == null) {
			ensureInitialized();
			if(internalSettings == null) return Fonts.REGULAR;
		}
		
		if(internalSettings.getMCHUDFont().isToggled()) {
			return Fonts.MOJANGLES;
		}
		
		switch(in) {
			case 1: return Fonts.REGULAR;
			case 2: return Fonts.MEDIUM;
			case 3: return Fonts.SEMIBOLD;
			default: return Fonts.REGULAR;
		}
	}
}