...
public class HypixelMod extends Mod {
	...
	private static final Pattern ANTI_L_REGEX = Pattern.compile(".*\\b[Ll]+\\b.*");
	...
	public void onUpdate(EventUpdate event) {
		...
		if (scoreboard != null && scoreboard.getObjectiveInDisplaySlot(1) != null) {
			...
			if(title.contains("TNT RUN")) currentMode = HypixelGameMode.TNT_RUN;
			if(title.contains("BOW SPLEEF")) currentMode = HypixelGameMode.BOW_SPLEEF;
			if(title.contains("PVP RUN")) currentMode = HypixelGameMode.PVP_RUN;
			if(title.contains("TNT TAG")) currentMode = HypixelGameMode.TNT_TAG;
			if(title.contains("TNT WIZARDS")) currentMode = HypixelGameMode.TNT_WIZARDS;
		}
	...
	public void onReceivePacket(EventReceivePacket event) {
		...
		if(event.getPacket() instanceof S2FPacketSetSlot) {
			...
			if(stack != null && stack.getItem().equals(Items.paper) && (HypixelGameMode.isBedwars(currentMode) || HypixelGameMode.isTntGames(currentMode))) {
				sendNextGame();
				return;
			}
		}
		...
		if(event.getPacket() instanceof S02PacketChat) {
			...
			if (antiLSetting.isToggled()) {
				Matcher matcher = ANTI_L_REGEX.matcher(chatMessage);
				event.setCancelled(matcher.find());
			}

			if (autoglSetting.isToggled() && chatMessage.contains("The game starts in 5")) {
				Multithreading.schedule(() -> {
					if (mc.thePlayer != null) mc.thePlayer.sendChatMessage("/achat gl");
		...
		if(event.getPacket() instanceof S45PacketTitle) {
			...
			if(titlePacket.getMessage() != null) {
				...
				if (autoggSetting.isToggled() && title.startsWith("\2476\247l") && title.endsWith("\247r")) {
					Multithreading.schedule(() -> {
						if (mc.thePlayer != null) mc.thePlayer.sendChatMessage("/achat gg");
				...
				if (itemID == 381 || itemID == 368) {
					if (itemname.contains("SkyWars")) {
						if (itemname.contains("Doubles")) {
							if (itemname.contains("Normal")) currentMode = HypixelGameMode.SKYWARS_DOUBLES_NORMAL;
							else if (itemname.contains("Insane")) currentMode = HypixelGameMode.SKYWARS_DOUBLES_INSANE;
						} else if (itemname.contains("Solo")) {
							if (itemname.contains("Normal")) currentMode = HypixelGameMode.SKYWARS_SOLO_NORMAL;
							else if (itemname.contains("Insane")) currentMode = HypixelGameMode.SKYWARS_SOLO_INSANE;
						}
					}
				} else if (itemID == 355) {
					if (itemname.contains("Bed Wars")) {
						if (itemname.contains("4v4")) currentMode = HypixelGameMode.BEDWARS_4V4;
						else if (itemname.contains("3v3")) currentMode = HypixelGameMode.BEDWARS_3V3;
						else if (itemname.contains("Doubles")) currentMode = HypixelGameMode.BEDWARS_DOUBLES;
						else if (itemname.contains("Solo")) currentMode = HypixelGameMode.BEDWARS_SOLO;
					}
				} else if(itemID == 397) {
					if(itemname.contains("UHC Duel")) {
						if(itemname.contains("1v1")) currentMode = HypixelGameMode.UHC_DUEL_1V1;
						else if(itemname.contains("2v2")) currentMode = HypixelGameMode.UHC_DUEL_2V2;
						else if(itemname.contains("4v4")) currentMode = HypixelGameMode.UHC_DUEL_4V4;
						else if(itemname.contains("Player FFA")) currentMode = HypixelGameMode.UHC_DUEL_MEETUP;
					}
				}
			}
		}
	...
	private void sendNextGame() {
		if(autoPlaySetting.isToggled()) {
			Multithreading.schedule(()-> {
				if (mc.thePlayer != null) mc.thePlayer.sendChatMessage(currentMode.getCommand());
			}, autoPlayDelaySetting.getValueInt(), TimeUnit.SECONDS);
		}
	}

	public static HypixelMod getInstance() { return instance; }
}