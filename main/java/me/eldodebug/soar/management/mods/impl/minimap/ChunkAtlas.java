...
public class ChunkAtlas implements Iterable<ChunkTile> {
	...
	// ✅ FIX: Track texture ID to prevent leak
	...
	public ChunkAtlas(int maxChunkRadius) {
		...
		try {
			...
			GL11.glTexImage2D(GL11.GL_TEXTURE_2D, 0, GL11.GL_RGBA, texWidth, texHeight, 0, GL12.GL_BGRA, GL12.GL_UNSIGNED_INT_8_8_8_8_REV, (IntBuffer) null);
			
			this.pixels = GLAllocation.createDirectIntBuffer(256);
			
			GlideLogger.info("[MINIMAP] ChunkAtlas created: " + texWidth + "x" + texHeight + ", texture: " + texture);
		} catch (Exception e) {
			GlideLogger.error("[MINIMAP] Failed to create ChunkAtlas", e);
			throw e;
		}
	}
	...
	public void delete() {
		if (texture > -1) {
			try {
				...
				GlideLogger.info("[MINIMAP] Deleted ChunkAtlas texture: " + texture);
			} catch (Exception e) {
				GlideLogger.error("[MINIMAP] Error deleting texture", e);
			} finally {
				texture = -1;
			}
		}
		...
		// Clear arrays
		Arrays.fill(chunkCoords, null);
		reusableChunks.clear();
	}
	...
	public void loadChunks(int chunkX, int chunkZ) {
		...
		for (int offs = 0; offs < this.chunkCoords.length; offs++) {
			...
			if (offsX < -this.radius || this.radius <= offsX || offsZ < -this.radius || this.radius <= offsZ) {
		...
		for (int relZ = this.radius * 2 - 1; relZ >= 0; relZ--) {
			for (int relX = this.radius * 2 - 1; relX >= 0; relX--) {
				...
				if (this.reusableChunks.get(checkIdx)) continue;
	...
	private void reserveOffset(Chunk c) {
		...
		if (offs == -1) {
			GlideLogger.warn("[MINIMAP] Chunk coordinate array full");
			return;
		}
	...
	private void updateColorData(Chunk src, int offs) {
		...
		try {
			...
			GlStateManager.bindTexture(this.texture);
			GL11.glTexSubImage2D(GL11.GL_TEXTURE_2D, 0, x, y, 16, 16, GL12.GL_BGRA, GL12.GL_UNSIGNED_INT_8_8_8_8_REV, this.pixels);
		} catch (Exception e) {
			GlideLogger.error("[MINIMAP] Error updating color data", e);
		}
	}
	...
	private void computeColors(Chunk src) {
		...
		for (int x = 0; x < 16; x++) {
			...
			for (int z = 0; z < 16; z++) {
				...
				if (northHeight > height) shade = 0;
				else if (northHeight >= 0 && northHeight < height) shade = 2;

				int depth = 0;
				BlockPos.MutableBlockPos checkPos = new BlockPos.MutableBlockPos(pos.getX(), pos.getY(), pos.getZ());
				
				while (checkPos.getY() >= 0 && !state.getBlock().getMaterial().isSolid()) {
					checkPos.set(checkPos.getX(), checkPos.getY() - 1, checkPos.getZ());
					state = src.getBlockState(checkPos);
					depth++;
				}
				...
				if (depth > 0) {
					...
					if (dither < 5) shade = 2;
					else if (dither > 9) shade = 0;
				}

				int rgb;
				if (height > 0) rgb = color.getMapColor(shade);
				else if (((x ^ z) & 3) == 0) rgb = 0x2d2d5a;
				else rgb = 0x1e1e3c;
	...
	private BlockPos getTopColoredBlockState(Chunk src, int x, int z) {
		if (src == null) return new BlockPos(x, 0, z);
		
		BlockPos.MutableBlockPos pos = new BlockPos.MutableBlockPos();
		int topY = src.getTopFilledSegment() + 15;
		
		for (int y = topY; y >= 0; y--) {
			IBlockState state = src.getBlockState(pos.set(x, y, z));
			if (state != null && state.getBlock().getMapColor(state) != MapColor.airColor) {
				break;
			}
		}
		return pos;
	...
	private Chunk getLoadedChunk(int x, int z) {
		...
		if (!((IMixinWorld)world).isLoaded(x, z, true)) return null;
