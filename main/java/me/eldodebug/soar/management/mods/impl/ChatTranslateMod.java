...
public void onReceivePacket(EventReceivePacket event) {
	...
	if(event.getPacket() instanceof S02PacketChat) {
		...
		if (chatPacket == null || chatPacket.getChatComponent() == null) return;
		
		IChatComponent chatComponent = chatPacket.getChatComponent();
		String chatMessage = chatComponent.getUnformattedText();
		
		if (chatMessage == null || chatMessage.trim().isEmpty() || chatPacket.getType() == 2) {
			return;
		}
		
		IChatComponent translate = new ChatComponentText(" [" + String.valueOf('\u270E') + "]")
		...
		mc.ingameGUI.getChatGUI().printChatMessage(chatComponent.appendSibling(translate));
	}
}
