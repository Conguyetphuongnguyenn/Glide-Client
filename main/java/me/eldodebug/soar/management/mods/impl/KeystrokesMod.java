...
public class KeystrokesMod extends HUDMod {
	...
	public KeystrokesMod() {
	    ...
	    for (int i = 0; i < 5; i++) animations[i] = new SimpleAnimation();
	}
	...
	private void drawNanoVG() {
		...
		if(spaceSetting.isToggled()) {
			...
			this.restore();
			if(!unmarkedSetting.isToggled()) this.drawRoundedRect(10, 74F, (26 * 3) - 6, 2, 1);
		}
	...
	private void drawHighlight(float addX, float addY, float width, float height, float radius, Color color){
		boolean rect = me.eldodebug.soar.management.mods.impl.InternalSettingsMod.getInstance().getModThemeSetting().getOption().getTranslate().equals(TranslateText.RECT);
