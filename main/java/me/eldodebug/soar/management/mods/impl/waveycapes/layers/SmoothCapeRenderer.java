...
public class SmoothCapeRenderer {
    ...
    public void renderSmoothCape(CustomCapeRenderLayer layer, AbstractClientPlayer abstractClientPlayer, float delta) {
        ...
        for (int part = 0; part < CustomCapeRenderLayer.partCount; part++) {
            ...
            if (oldPositionMatrix == null) oldPositionMatrix = poseStack.last().getPose();

            if (part == 0) {
                addTopVertex(worldrenderer, poseStack.last().getPose(), oldPositionMatrix, 0.3F, 0, 0F, -0.3F, 0, -0.06F, part);
            }

            if (part == CustomCapeRenderLayer.partCount - 1) {
                addBottomVertex(worldrenderer, poseStack.last().getPose(), poseStack.last().getPose(), 0.3F, (part + 1) * (0.96F / CustomCapeRenderLayer.partCount), 0F, -0.3F, (part + 1) * (0.96F / CustomCapeRenderLayer.partCount), -0.06F, part);
            }

            addLeftVertex(worldrenderer, poseStack.last().getPose(), oldPositionMatrix, -0.3F, (part + 1) * (0.96F / CustomCapeRenderLayer.partCount), 0F, -0.3F, part * (0.96F / CustomCapeRenderLayer.partCount), -0.06F, part);
            addRightVertex(worldrenderer, poseStack.last().getPose(), oldPositionMatrix, 0.3F, (part + 1) * (0.96F / CustomCapeRenderLayer.partCount), 0F, 0.3F, part * (0.96F / CustomCapeRenderLayer.partCount), -0.06F, part);
            addBackVertex(worldrenderer, poseStack.last().getPose(), oldPositionMatrix, 0.3F, (part + 1) * (0.96F / CustomCapeRenderLayer.partCount), -0.06F, -0.3F, part * (0.96F / CustomCapeRenderLayer.partCount), -0.06F, part);
            addFrontVertex(worldrenderer, oldPositionMatrix, poseStack.last().getPose(), 0.3F, (part + 1) * (0.96F / CustomCapeRenderLayer.partCount), 0F, -0.3F, part * (0.96F / CustomCapeRenderLayer.partCount), 0F, part);

            oldPositionMatrix = poseStack.last().getPose();
            poseStack.popPose();
        }
    ...
    private void modifyPoseStackVanilla(CustomCapeRenderLayer layer, PoseStack poseStack, AbstractClientPlayer abstractClientPlayer, float h, int part) {
        poseStack.pushPose();
        poseStack.translate(0.0D, 0.0D, 0.125D);
        double d = MathUtils.lerp(h, abstractClientPlayer.prevChasingPosX, abstractClientPlayer.chasingPosX) - MathUtils.lerp(h, abstractClientPlayer.prevPosX, abstractClientPlayer.posX);
        double e = MathUtils.lerp(h, abstractClientPlayer.prevChasingPosY, abstractClientPlayer.chasingPosY) - MathUtils.lerp(h, abstractClientPlayer.prevPosY, abstractClientPlayer.posY);
        double m = MathUtils.lerp(h, abstractClientPlayer.prevChasingPosZ, abstractClientPlayer.chasingPosZ) - MathUtils.lerp(h, abstractClientPlayer.prevPosZ, abstractClientPlayer.posZ);
    ...
    private static void addBackVertex(WorldRenderer worldrenderer, Matrix4f matrix, Matrix4f oldMatrix, float x1, float y1, float z1, float x2, float y2, float z2, int part) {
        ...
        float minU = .015625F; float maxU = .171875F; float minV = .03125F; float maxV = .53125F;
        float deltaV = maxV - minV; float vPerPart = deltaV / CustomCapeRenderLayer.partCount;
        maxV = minV + (vPerPart * (part + 1)); minV = minV + (vPerPart * part);
    ...
    private static void addFrontVertex(WorldRenderer worldrenderer, Matrix4f matrix, Matrix4f oldMatrix, float x1, float y1, float z1, float x2, float y2, float z2, int part) {
        ...
        if (x1 < x2) { i = x1; x1 = x2; x2 = i; }
        if (y1 < y2) { i = y1; y1 = y2; y2 = i; k = matrix; matrix = oldMatrix; oldMatrix = k; }
        float minU = .1875F; float maxU = .34375F; float minV = .03125F; float maxV = .53125F;
        float deltaV = maxV - minV; float vPerPart = deltaV / CustomCapeRenderLayer.partCount;
        maxV = minV + (vPerPart * (part + 1)); minV = minV + (vPerPart * part);
    ...
    private static void addLeftVertex(WorldRenderer worldrenderer, Matrix4f matrix, Matrix4f oldMatrix, float x1, float y1, float z1, float x2, float y2, float z2, int part) {
        ...
        float minU = 0; float maxU = .015625F; float minV = .03125F; float maxV = .53125F;
        float deltaV = maxV - minV; float vPerPart = deltaV / CustomCapeRenderLayer.partCount;
        maxV = minV + (vPerPart * (part + 1)); minV = minV + (vPerPart * part);
    ...
    private static void addRightVertex(WorldRenderer worldrenderer, Matrix4f matrix, Matrix4f oldMatrix, float x1, float y1, float z1, float x2, float y2, float z2, int part) {
        ...
        if (x1 < x2) { i = x1; x1 = x2; x2 = i; }
        if (y1 < y2) { i = y1; y1 = y2; y2 = i; }
        float minU = .171875F; float maxU = .1875F; float minV = .03125F; float maxV = .53125F;
        float deltaV = maxV - minV; float vPerPart = deltaV / CustomCapeRenderLayer.partCount;
        maxV = minV + (vPerPart * (part + 1)); minV = minV + (vPerPart * part);
        vertex(worldrenderer, matrix, x2, y1, z2).tex(minU, maxV).normal(1, 0, 0).endVertex();
        vertex(worldrenderer, matrix, x2, y1, z1).tex(maxU, maxV).normal(1, 0, 0).endVertex();
    ...
    private static void addBottomVertex(WorldRenderer worldrenderer, Matrix4f matrix, Matrix4f oldMatrix, float x1, float y1, float z1, float x2, float y2, float z2, int part) {
        ...
        if (x1 < x2) { i = x1; x1 = x2; x2 = i; }
        if (y1 < y2) { i = y1; y1 = y2; y2 = i; }
        float minU = .171875F; float maxU = .328125F; float minV = 0; float maxV = .03125F;
        float deltaV = maxV - minV; float vPerPart = deltaV / CustomCapeRenderLayer.partCount;
        maxV = minV + (vPerPart * (part + 1)); minV = minV + (vPerPart * part);
    ...
    private static void addTopVertex(WorldRenderer worldrenderer, Matrix4f matrix, Matrix4f oldMatrix, float x1, float y1, float z1, float x2, float y2, float z2, int part) {
        ...
        if (x1 < x2) { i = x1; x1 = x2; x2 = i; }
        if (y1 < y2) { i = y1; y1 = y2; y2 = i; }
        float minU = .015625F; float maxU = .171875F; float minV = 0; float maxV = .03125F;
        float deltaV = maxV - minV; float vPerPart = deltaV / CustomCapeRenderLayer.partCount;
        maxV = minV + (vPerPart * (part + 1)); minV = minV + (vPerPart * part);
