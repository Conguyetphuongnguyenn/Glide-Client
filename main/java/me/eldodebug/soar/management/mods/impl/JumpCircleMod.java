	...
	public void onRender3D(EventRender3D event) {
        ...
        for (JumpCircle circle : circles) {
            ...
            for (int i = 0; i <= 360; i += 5) {
                ...
                Vec3 pos = circle.pos();
                double anim = circle.getAnimation(((IMixinMinecraft)mc).getTimer().renderPartialTicks);
                double x = Math.cos(Math.toRadians(i)) * createAnimation(1.0 - anim) * 0.7;
                double z = Math.sin(Math.toRadians(i)) * createAnimation(1.0 - anim) * 0.7;
                
                GL11.glColor4d(red, green, blue, 0.6 * anim);
                GL11.glVertex3d(pos.xCoord + x, pos.yCoord + 0.2, pos.zCoord + z);
                GL11.glColor4d(red, green, blue, 0.2 * anim);
