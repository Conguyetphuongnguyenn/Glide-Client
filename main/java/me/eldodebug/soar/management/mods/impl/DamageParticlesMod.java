...
public class DamageParticlesMod extends Mod {
	...
	public void onTick(EventTick event) {
		if(canRemove) particles.remove(removeParticle);
		
		particles.forEach(particle -> {
			particle.ticks++;
			if (particle.ticks <= 10) particle.location.setY(particle.location.getY() + particle.ticks * 0.005);
	...
	public void onLivingUpdate(EventLivingUpdate event) {
		...
		if (!healthMap.containsKey(entity)) healthMap.put(entity, entity.getHealth());
		...
		if (before != after) {
			String text;
			if ((before - after) < 0) text = EnumChatFormatting.GREEN + "" + MathUtils.roundToPlace((before - after) * -1, 1);
			else text = EnumChatFormatting.YELLOW + "" + MathUtils.roundToPlace((before - after), 1);
			...
			location.setX((location.getX() - 0.5) + (RANDOM.nextInt(5) * 0.1));
			location.setZ((location.getZ() - 0.5) + (RANDOM.nextInt(5) * 0.1));
