...
public class WaypointMod extends Mod {
	...
	public void onRender3D(EventRender3D event) {
		...
		for(Waypoint wy : instance.getWaypointManager().getWaypoints()) {
			if(instance.getWaypointManager().getWorld().equals(wy.getWorld())) {
	...
	public void onKey(EventKey event) {
		if(event.getKeyCode() == keybindSetting.getKeyCode()) mc.displayGuiScreen(new GuiWaypoint());
	}
