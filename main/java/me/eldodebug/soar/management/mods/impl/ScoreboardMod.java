...
public class ScoreboardMod extends HUDMod {
	...
	// Cached colors (tránh allocations)
	...
	// Cache (tránh StringBuilder allocations)
	...
	public void onRender2D(EventRender2D event) {
		...
		// Clear objective nếu singleplayer
		...
		// FIX: Dùng filteredScores.size() thay vì scores.size()
		Collection<Score> displayScores;
		if (filteredScores.size() > MAX_SCORES) {
			displayScores = Lists.newArrayList(
				Iterables.skip(filteredScores, filteredScores.size() - MAX_SCORES)
			);
		} else {
			displayScores = filteredScores;
		}
		
		// OPTIMIZED: Calculate maxWidth 1 lần
		...
		for (Score score : displayScores) {
			...
			if (showNumbers) {
				// OPTIMIZED: Dùng StringBuilder thay vì concatenation
				scoreBuilder.setLength(0);
				scoreBuilder.append(": ")
					.append(EnumChatFormatting.RED)
					.append(score.getScorePoints());
		...
		if (nvg != null && shadowSetting.isToggled()) {
			nvg.setupAndDraw(() -> {
				this.drawShadow(0, 0, this.getWidth() / this.getScale(), 
					this.getHeight() / this.getScale(), 0);
			});
		}
		
		// OPTIMIZED: Cache values ngoài loop
		int posX = this.getX();
		int posY = this.getY();
		...
		// Header (render cuối cùng = render lên trên)
		if (index > 0) {
