...
public class EntityCullingMod extends Mod {
    ...
    // THREAD SAFE: AtomicInteger thay vì int
    private final AtomicInteger destroyTimer = new AtomicInteger(0);
    
    // PRE-ALLOCATED: Tái sử dụng thay vì new mỗi tick
    ...
    public void onDisable() {
        ...
        // CRITICAL: Reset static flag
        shouldPerformCulling = false;
        
        // CRITICAL: Cleanup GL resources
        cleanupAllQueries();
        
        // CRITICAL: Clear collections
        removeList.clear();
        loadedSet.clear();
    }
    ...
    public void onRendererLivingEntity(EventRendererLivingEntity event) {
        ...
        // NULL CHECKS FIRST
        if (!this.isToggled() || !shouldPerformCulling) {
            return;
        }
    ...
    public void onTick(EventTick event) {
        ...
        if (destroyTimer.incrementAndGet() < 120) {
            return;
        }
        ...
        // PERFORMANCE: Reuse pre-allocated collections
        removeList.clear();
        loadedSet.clear();
        
        // NULL CHECK: loadedEntityList có thể null
        if (theWorld.loadedEntityList == null) {
            return;
        }
        
        for (Entity entity : theWorld.loadedEntityList) {
            if (entity != null) {
                loadedSet.add(entity.getUniqueID());
            }
        }

        // SAFE ITERATION: Tránh ConcurrentModificationException
        try {
            for (OcclusionQuery value : queries.values()) {
                ...
                if (loadedSet.contains(value.uuid)) {
                    continue;
                }
            ...
            // Concurrent modification - skip this tick
        }
    }
    ...
    public boolean canRenderName(EntityLivingBase entity) {
        ...
        // NULL CHECK: renderManager.livingPlayer
    ...
    public boolean renderItem(Entity stack) {
        ...
        // NULL CHECKS
        if (stack == null || mc.thePlayer == null || mc.thePlayer.worldObj == null) {
            return false;
        }
        
        return shouldPerformCulling && 
               stack.worldObj == mc.thePlayer.worldObj && 
               checkEntity(stack);
    }
    ...
    private boolean checkEntity(Entity entity) {
        ...
        if (query.refresh) {
            ...
            // NULL CHECK: renderManager mixin
    ...
    public static void drawSelectionBoundingBox(AxisAlignedBB b) {
        ...
        try {
            ...
            // CRITICAL: Restore GL state
            GlStateManager.depthMask(true);
            GlStateManager.colorMask(true, true, true, true);
            GlStateManager.enableAlpha();
        } catch (Exception e) {
            // Rendering error - restore state anyway
            try {
                GlStateManager.depthMask(true);
                GlStateManager.colorMask(true, true, true, true);
                GlStateManager.enableAlpha();
