...
public class SoundSubtitlesMod extends HUDMod {
	...
	public SoundSubtitlesMod() {
        ...
        try (InputStream is = mc.getResourceManager().getResource(mapped).getInputStream()) {
            JsonObject obj = new JsonParser().parse(read(is)).getAsJsonObject();
	...
	private void drawNanoVG() {
        ...
        for(Subtitle subtitle : subtitles) {
            if (subtitle.getStartTime() + 3000L <= Minecraft.getSystemTime()) subtitle.setRemove(true);
            if(subtitle.isRemove() && subtitle.isDone()) removeList.add(subtitle);
        }
		...
		if(backgroundAnimation.getValue() > 1) {
	        ...
	        for(Subtitle subtitle : (this.isEditing() ? fakeSubtitle : subtitles)) {
	            ...
	            if(subtitle.animation.getValue() < 0.1 && subtitle.isRemove()) subtitle.setDone(true);
            	...
            	if(index == 1) animationOffsetY = 0;
            	
            	this.drawCenteredText(subtitle.getString(), subtitleWidth / 2, animationOffsetY + 4, 9, getHudFont(1), this.getFontColor(opacity));

	            if (!flag) {
	                if (d0 > 0.0D) this.drawText(">", subtitleWidth - this.getTextWidth("<", 9, getHudFont(1)) - 4.5F, animationOffsetY + 4.5F, 9, getHudFont(1), this.getFontColor(opacity));
	                else if (d0 < 0.0D) this.drawText("<", 4.5F, animationOffsetY + 4.5F, 9, getHudFont(1), this.getFontColor(opacity));
	            }
	        	index++;
	        }
    ...
    public void soundPlay(ISound soundIn) {
    	if(subtitles.size() >= maxSetting.getValue()) return;
        String s = getSoundName(soundIn.getSoundLocation());
        if (s == null) s = soundIn.getSoundLocation().getResourcePath();
        if (s.isEmpty()) return;
	...
	public static SoundSubtitlesMod getInstance() { return instance; }
}