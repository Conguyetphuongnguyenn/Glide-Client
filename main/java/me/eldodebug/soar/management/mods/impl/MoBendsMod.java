...
public class MoBendsMod extends Mod {
	...
	public void onTick(EventTick event) {
        ...
        for(int i = 0;i < Data_Player.dataList.size();i++){
            ...
            }else Data_Player.dataList.remove(data);
        }
	}
	...
	public void onRenderPlayer(EventRenderPlayer event) {
        ...
        if(AnimatedEntity.getByEntity(event.getEntity()) == null) return;
	...
	public void onEnable() {
		super.onEnable();
		if(Skin3DMod.getInstance().isToggled()) Skin3DMod.getInstance().setToggled(false);
		if(WaveyCapesMod.getInstance().isToggled()) WaveyCapesMod.getInstance().setToggled(false);
		if(FemaleGenderMod.getInstance().isToggled()) FemaleGenderMod.getInstance().setToggled(false);
		if(!loaded) AnimatedEntity.register();
	}
	
	public static MoBendsMod getInstance() { return instance; }
	public ColorSetting getColorSetting() { return colorSetting; }
	public BooleanSetting getCustomColorSetting() { return customColorSetting; }
	public boolean isRenderingGuiScreen() { return renderingGuiScreen; }
	public void setRenderingGuiScreen(boolean renderingGuiScreen) { this.renderingGuiScreen = renderingGuiScreen; }
}